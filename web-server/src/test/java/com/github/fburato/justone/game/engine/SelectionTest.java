...
import static com.github.fburato.justone.game.engine.EngineTestUtils.removeHint;
    ...
    void failOnNotSelection() {
        final var gameStateInWrongPhase = gameStateBuilder(stateWithSelection)
                .with(gsb -> {
                    ...
                    gsb.turns.set(1, turnBuilder(stateWithSelection.turns().get(0)).with(
                            tb -> tb.phase = TurnPhase.CONCLUSION).build());
                }).build();
